...
public class DrawView extends Panel {
    ...
    public void update(Graphics g){
        if(iBuffer==null){
            iBuffer=createImage(DrawView.VIEW_WIDTH*DrawView.VIEW_NUMBER+1,
                    DrawView.VIEW_HEIGHT*DrawView.VIEW_NUMBER+1);
            gBuffer=iBuffer.getGraphics();
        }
        gBuffer.setColor(getBackground());
        gBuffer.fillRect(0,0,DrawView.VIEW_WIDTH*DrawView.VIEW_NUMBER+1,
                DrawView.VIEW_HEIGHT*DrawView.VIEW_NUMBER+1);
        paint(gBuffer);
    ...
    public void drawSnake(Graphics g){
        for(int i=0;i<snake.getSnakeBody().size();i++){
            ...
            g.fillRect(snake.getSnakeBody().get(i).getNodeX(),snake.getSnakeBody().get(i).getNodeY(),
